...
import logging
import time
from helper import is_match, is_match_and_bare, check_connection, get_commands
...
class SpeechAssistant(Configuration):
    ...
    def init_bot(self):
        try:
            ...
            pass
            self.Log(
                f"Error while initiating telegram bot. {ex}", logging.INFO)
            self.kill_tts_events()
    ...
    def speak(self, audio_string, start_prompt=False, end_prompt=False, mute_prompt=False):
        ...
        if self.restart_request:
            ...
            try:
                ...
                if isGTTS:
                    audio_file = f"{self.AUDIO_FOLDER}/assistants-audio-{str(randint(1, 1000))}.mp3"
                ...
                if start_prompt and "<start prompt>" in audio_string:
                    ...
                    elif len(removeUrl) <= 1000:
                        os.system(f"say \"{removeUrl}\"")
                    else:
                        os.system("say 'Here you go...'")
